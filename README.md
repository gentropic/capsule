...
## Vendoring the single-file bundle

For GCU siblings (and anyone wanting one self-contained file instead of the
`src/` module tree), the build emits a bundle — the whole package concatenated,
zero imports:

- **`dist/capsule.js`** — ESM (`export { … }`), also reachable as the
  `@gcu/capsule/bundled` export. `import` it, or inline it into a single-file build.
- **`dist/capsule.global.js`** — an IIFE that sets `globalThis.gcuCapsule = { … }`,
  for classic-`<script>` inlining (no module system needed).

```js
import * as capsule from '@gcu/capsule/bundled';   // npm consumers
// or, inlined into a single-file app:
<script src="capsule.global.js"></script>           // → window.gcuCapsule
```

Both files are **committed** (not just built on publish), so they're vendorable
with no toolchain. Pick whichever fits:

- **Build-time inline from a sibling clone** (the cradle pattern, fully offline):
  a sibling's build reads `../capsule/dist/capsule.js` and inlines it — single
  source, no runtime fetch. Pin to a commit/tag.
- **jsDelivr**, pinned: `cdn.jsdelivr.net/npm/@gcu/capsule@<ver>/dist/capsule.js`
  or `…/gh/gentropic/capsule@<sha>/dist/capsule.js`.
- **Copy the file in** (like vendoring pako) and update on bumps.

The bundle is generated by `build/bundle.js` (zero-dependency) and **must not be
hand-edited** — `npm run build:check` (run in CI) fails if `dist/` drifts from
`src/`. Edit the source, run `npm run build`, commit both.
