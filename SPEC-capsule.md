...
- decode with a **single left-to-right pass** that recognizes the two triplets `%25` → `%` and `%20` → space and copies every other character verbatim. A decoder MUST NOT use repeated/global string replacement: replacing `%25` → `%` and then `%20` → space in two passes corrupts a literal `%20` already present in the raw payload — it encodes to `%2520`, the first pass turns it into `%20`, and the second pass wrongly turns *that* into a space, when it must round-trip back to `%20`. A decoder also MUST NOT delegate to a general `decodeURIComponent`. For *conformant* payloads `decodeURIComponent` in fact produces identical output (the encoder leaves behind only `%25` and `%20` triplets, both of which it decodes correctly, and it does not touch the fragment-legal `+`, `$`, `*`, `/`); the prohibition is for robustness against malformed or adversarial fragments, where `decodeURIComponent` throws a `URIError` on a lone `%` and eagerly decodes any other `%HH` a non-conformant producer might emit, whereas the single-pass decoder degrades gracefully — copying anything that is not exactly `%25`/`%20` verbatim.
